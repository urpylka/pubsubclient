...
// НОВЫЕ ПОДКЛЮЧЕНИЯ ДЛЯ ОЧЕРЕДИ И УМНЫХ УКАЗАТЕЛЕЙ
#include <queue>
#include <memory>
#include <vector>
...
// >> НАЧАЛО: НОВЫЕ ОПРЕДЕЛЕНИЯ ДЛЯ ОЧЕРЕДИ
...
// << КОНЕЦ: НОВЫЕ ОПРЕДЕЛЕНИЯ ДЛЯ ОЧЕРЕДИ
...
private:
   ...
   // НОВАЯ ОЧЕРЕДЬ для исходящих сообщений
   std::queue<std::unique_ptr<MqttOutgoingMessage>> outgoingQueue;
   // НОВЫЙ приватный метод для отправки из очереди
