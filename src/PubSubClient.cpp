...
boolean PubSubClient::loop()
{
    if (connected())
    {
        // НОВЫЙ ШАГ: Отправить одно сообщение из очереди, если оно есть
        sendFromQueue();

        // СТАРЫЙ КОД: проверка keep-alive
        unsigned long t = millis();
        ...
        // СТАРЫЙ КОД: обработка входящих данных
            ...
            if (len > 0)
            {
                ...
                if (type == MQTTPUBLISH)
                {
                    if (callback)
                    {
                        ...
                        {
                            payload = this->buffer + llen + 2 + tl + 1;
                            callback(topic, payload, len - llen - 2 - tl - 1);
                        }
                    }
                }
...
// ===================================================================
//
//      ОСТАЛЬНЫЕ ФУНКЦИИ БИБЛИОТЕКИ (БЕЗ ИЗМЕНЕНИЙ)
//
// ===================================================================
...
boolean PubSubClient::publish_P(const char *topic, const uint8_t *payload, unsigned int plength, boolean retained)
{
    ...
    return (rc == (1 + llen + 2 + tlen + plength));
}
